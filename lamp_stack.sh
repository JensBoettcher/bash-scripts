...
check_resource_group_name() {
    ...
    if [ ${#name} -gt 90 ]; then
        echo "The maximal name length is 90 characters."
        return 1
    fi
    return 0
}
...
#NOTE - function to check the admin-username
check_user_name() {
    local user_name="$1"
    #check name is empty
    if [ -z "$user_name" ]; then
...
#ANCHOR - read the users resource-group-name
while true; do
...
#ANCHOR - fingerprint query automation and LAMP stack installation
#NOTE - It is not recommend to automate the fingerprint confirmation for safety reasons
